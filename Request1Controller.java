...
public class Request1Controller implements Initializable {
    ...
    @FXML public TableView<Request1> table;
    @FXML public TableColumn<Request1, String> tableFirstName;
    @FXML public TableColumn<Request1, String> tableReaderID;
    @FXML public TableColumn<Request1, String> tableLastName;
    @FXML public TableColumn<Request1, String> tableType;
    @FXML public Text TextCount;
    private final ObservableList<Request1> data = FXCollections.observableArrayList();
    ...
    public void addInfo() {
        ...
        switch (fnProperty.getSelectionModel().getSelectedItem().toString()) {
            case "id абонемента": {
                str = "SELECT readers.id, readers.first_name, readers.last_name, readers.type\n" +
                        ...
                        "where lib_cards.location_id = " + fnPropValue.getText();
                break;
            }
            case "группа": {
                str = "SELECT readers.id, readers.first_name, readers.last_name, readers.type\n" +
        ...
        try {
            while (readers.next()) {
                Request1 request1 = new Request1(readers.getString(1),
                        readers.getString(2),
                        readers.getString(3),
                        readers.getString(4));
                data.add(request1);
